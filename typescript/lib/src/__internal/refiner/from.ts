...
export const dictionary = <T extends p_di.Value>(
    ...
    return {
        ...
        ): p_di.Dictionary<New_Type> => new Dictionary_Class<New_Type>(dict.__get_raw().map(([id, value]) => {
            return [
                id,
                assign_entry(value, id)
            ]
        })),

        map_and_aggregate_error: <
            New_Type extends p_di.Value,
            Entry_Error extends p_di.Value,
        >(
            assign_entry: (
                value: T,
                id: string,
                abort: Abort<Entry_Error>
            ) => New_Type,
            on_encountered_errors: Abort<p_di.Dictionary<Entry_Error>>
        ): p_di.Dictionary<New_Type> => {
            const errors: [string, Entry_Error][] = []
            const results: [string, New_Type][] = []
                    ...
                    results.push([
                        ...
                        assign_entry(
                            ...
                            (error) => {
                                ...
                                throw new My_Error(`Error processing entry with id ${id}`)
                            }
                        )
                    ])
                } catch (e) {
            ...
            if (errors.length !== 0) {
                return on_encountered_errors(new Dictionary_Class(errors))
            } else {
                return new Dictionary_Class(results)
            }
        },
...
export const list = <T extends p_di.Value>(
    ...
    return {
        ...
        >(
            ...
            on_encountered_errors: Abort<p_di.List<Entry_Error>>
        ): p_di.List<New_Type> => {
            const errors: Entry_Error[] = []
            const results: New_Type[] = []
            list.__get_raw().forEach((value) => {
                ...
                try {
                    results.push(
                        assign_element(
                            ...
                            (error) => {
                                errors.push(error)
                                throw new My_Error(`Error processing element`)
                            }
                        )
                    )
                } catch (e) {
            ...
            if (errors.length !== 0) {
                return on_encountered_errors(lit.list(errors))
            } else {
                return lit.list(results)
            }
        },
