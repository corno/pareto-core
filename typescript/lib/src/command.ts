...
export namespace decide {
    ...
    export function ss<T extends _pi.Value, RT extends _pi.Command_Promise<any>>(
        option: Option<T>,
        $c: ($: T) => RT): RT {
        return $c(option[1])
    }

    export function au<RT extends _pi.Command_Promise<any>>(
        _x: never
    ): RT {
    ...
    export const state = <
        T extends _pi.State,
        RT extends _pi.Command_Promise<any>
    >(
        state: T,
        assign: (output: T) => RT
    ): RT => {
